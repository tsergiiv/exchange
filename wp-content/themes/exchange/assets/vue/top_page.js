...
var vMainpage = new Vue({
    ...
    watch: {
      ...
      selected_currency: function(val, oldVal) {
        // this.selected_currency_code = this.options_currencys.find(option => option.code === val).code;
        if (!this.last_selected_currency_code) {
          this.last_selected_currency_code = val;
        }
    ...
    methods: {
              ...
              axios
                ...
                .then(response => {
                    ...
                    if(this.list_options_currencys.find(option => option.value === response.data.country).code != 'EUR' && this.list_options_currencys.find(option => option.value === response.data.country).code != 'USD') {
                      ...
                      selected_currency = this.list_options_currencys.find(option => option.value === response.data.country).code
                    }
                });
            }
        ...
        getExchanges: function() {
          ...
          axios
            .get('http://api.ers.takasho.work/exchange-rates?countryCode='+this.selected_country+'&cryptoCurrencyCode='+this.selected_destination_coin+'&fiatCurrencyCode='+this.selected_currency+'&depositMethodType='+this.selected_deposit_method+'&amount='+this.amount)
            .then(response => {
              if (response.data) {
