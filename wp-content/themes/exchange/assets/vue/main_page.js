...
var vMainpage = new Vue({
    ...
    watch: {
      selected_country: function(val, oldVal) {
        if(this.list_options_currencys.find(option => option.value === val).code != 'EUR' && this.list_options_currencys.find(option => option.value === val).code != 'USD') {
          this.selected_currency = 'EUR';
        } else {
          this.selected_currency = this.list_options_currencys.find(option => option.value === val).code
        }
        this.selected_country_name = this.countrys.find(option => option.value === val).name;
        this.setCookieData();
      },
    ...
    methods: {
        ...
        //           this.test.push({
        //             value: response.data[index].alpha2Code,
        //             name: response.data[index].currencies[0].name,
        //             code: response.data[index].currencies[0].code,
        //             flag: response.data[index].flag
        //           });
        //         }
        //       }
    ...
    mounted() {
        ...
        axios
            ...
            .then(response => {
                ...
                if(this.list_options_currencys.find(option => option.value === response.data.country).code != 'EUR' && this.list_options_currencys.find(option => option.value === response.data.country).code != 'USD') {
                  this.selected_currency = 'EUR';
                } else {
                  this.selected_currency = this.list_options_currencys.find(option => option.value === response.data.country).code
                }
            });
        this.setCookieData();
        this.customSelectInit();
